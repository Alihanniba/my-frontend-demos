...
//获取所有子账号
addAd.getChildAccount = function() {
	$.ajax({
...
addAd.renderPiriodSelector = function(dom) {
	var html = [];
...
addAd.bindStepEvent = function() {
	if($(".step-process>li").length > 0) {
		checkStep(addAd.step);
...
addAd.renderPage = function() {
	...
	.done(function(data) {
		if(data.success) {
			addAd.zoneTypes = data.result.areaJson;
			addAd.sceneTypes = data.result.sceneJson;
			addAd.scenesMessage = data.result.scenesMessage;
			...
			addAd.bindSelectEvent();
			addAd.getRegionSelector("#region-selector-box", addAd.zoneTypes);
			addAd.getZoneDropdown();
			addAd.bindRegionEvent();

			addAd.getSceneSelector("#scene-selector-box", addAd.sceneTypes);
			addAd.getDiySceneSelector("#scene-diy-selector-box", addAd.scenesMessage);
			addAd.bindSceneEvent();
...
addAd.bindSelectEvent = function() {
	...
	$("#img-review").on("click", ".close-btn", function() {
		...
		addAd.imgUrls.img.splice($(this).parents("li").index(), 1);
		console.log(addAd.imgUrls.img);
	});
	...
	$("#daily-budget").on("blur", function() {
		if(!isNaN($(this).val()) && parseInt($(this).val()) < 10) {
			layer.alert("请输入大于等于10的数字，保留两位小数");
			return;
		}
...
addAd.getRegionSelector = function(dom, data) {
	var province = data;
	var html = "";
	var reviewHtml = "";
	if(province != undefined) {
		for(var i = 0, ilen = province.length; i < ilen; i++) {
			...
			var isDirect = addAd.isDirect(province[i].text);
...
addAd.getZoneSelector = function(dom, data) {
...
addAd.getZoneDropdown = function() {
	...
	//addAd.getZoneList(addAd.zoneTypes[0]);
	this.bindZoneEvent();
}

addAd.getZoneCityDropdown = function(data) {
	...
	//addAd.getZoneList(city[0]);
	this.bindZoneCityEvent(city);
}

addAd.bindZoneEvent = function() {
	$(document).on("click", "#province-dropdown .dropdown-menu>li", function() {
		var data = addAd.zoneTypes[$(this).index()];
		if(addAd.isDirect(data.text)) {
			addAd.getZoneList(data);
			$("#city-dropdown").hide();
		} else {
			addAd.getZoneCityDropdown(data);
			$("#city-dropdown").show();
		}
...
addAd.bindZoneCityEvent = function (cityData) {
...
addAd.bindRegionEvent = function() {
	...
	$(".region-select-list").on("change", ".region-lv2", function() {
		if($(this).prop("checked")) {
			...
			if($(this).parents(".city-select-list").find(".region-lv2:checked").length == 0) {
				$(this).parents(".city-select-list").parent().find(".region-lv1").prop("checked", false).removeClass("check-half");
			}
			$(this).parents(".region-select-list").siblings(".region-select-all").find("input").prop("checked", false);
			//review
			$(this).parents(".region-selector-box").find(".region-review-list li[data-rid=" + $(this).attr("id") + "]").removeClass("review-show");
		...
		if($(this).parents(".city-select-list").find(".region-lv2").length != $(this).parents(".city-select-list").find(".region-lv2:checked").length) {
...
addAd.getDiySceneSelector = function(dom, data) {
	var parents = data;
	var html = "";
	var reviewHtml = "";
	for(var i = 0, ilen = parents.length; i < ilen; i++) {
...
addAd.bindSceneEvent = function() {
	$(".tree-select .parent-name").on("click", function() {
	...
	$(".tree-select .child-name").on("click", function() {
	...
	$(document).on("click", ".scene-children-review-list>li", function(event) {
		...
		init: {
			...
			'FileUploaded': function(up, file, info) {
				...
				if(checkImgPx(imageInfoObj.width, imageInfoObj.height)) {
					...
					addAd.imgUrls.img.push({
						'url': url,
						'size': file.size
					});
					//console.log(file);
					var size = file.size > 1000 ? file.size / 1000 + "KB" : file.size + "B";
					var html = '<li class="clearfix"><div class="img-review-pic fl"><img src="' + url + '"/></div><div class="img-review-name fl">' + file.name + '</div><div class="img-review-size fl">' + size+ '</div><div class="fa fa-close close-btn"></div></li>';
...
addAd.bindDataEvent = function() {
	...
	var fd = $.extend({}, formData, dateData, addAd.imgUrls, accountData, piriodData);
...
addAd.reviewData = function() {
	var html = '';
	var result = {};
	var dateHtml = '<p>推广日期：' + $("#startDate").val();
	if($("#hasTimeEnd").prop("checked") && $("#endDate").val() != "") {
		dateHtml += ' 至 ' + $("#endDate").val() + '</p>';
	} else {
		dateHtml += ' 起</p>';
	}
	...
	html += accountHtml;
	var dom = '';
	$(".add-ad-step:lt(" + addAd.step + ")  .form-group input").each(function() {
		if($(this).prop("disabled")) {
			return;
		}
		var type = $(this).attr("type");
		...
		if(name) {
			if(type.toLocaleLowerCase() == "checkbox") {
				...
				if($(".form-group input[type='checkbox'][name='" + name + "']:checked").length == result[name].length) {
					html += '<p>' + $(this).parents(".form-group").children("label").text() + '：' + result[name].join(",") + '</p>';
				}
			} else if (type.toLocaleLowerCase() == "radio") {
				html += '<p>' + $(this).parents(".form-group").children("label").text() + '：' + $(this).siblings("span").text() + '</p>';
			} else {
				if(name == "add-ad-img") {
					return;
				}
				html += '<p>' + $(this).parents(".form-group").children("label").text() + '：' + $(this).val() + '</p>';
