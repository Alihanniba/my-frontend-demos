...
//        echo $html;
        if (strpos($url, "#")) {
            ...
            preg_match("/vid:[']?([\d]+)/s", $html, $vids);
            preg_match("(pic\s*:\s*'(.*)')", $html, $imgs);
            if (is_array($vids) && !empty($vids[1])) {
                $vid_array = explode("|", trim($vids[1])); //第二个vid为高清格式的vid
            } else if (preg_match("/video_id=([\d]+)/s", $url, $v)) {
                $video_id = $v[1];
                $j = file_get_contents("http://s.video.sina.com.cn/video/play?video_id=" . $video_id . "&player=flash&0.6903a05583041906=");
                $jObj = json_decode($j);
                if (isset($jObj->data->videos[0]->file_id)) {
                    $vid_array[0] = $jObj->data->videos[0]->file_id;
                } else {
                    return array (
                        'status' => false
                    );
        ...
        $vid_ipad_array = json_decode(file_get_contents($api_ipad_url), true);
...
//        $data['normal'][] = "http://v.iask.com/v_play_ipad.php?vid=" . $vid_array[0];
        $data['seconds'] = intval($video_data['seconds'] ? $video_data['seconds'] : '');

        $segaments = array();
        $segaments[] = array(
            'seconds' => intval($video_data['seconds'] ? $video_data['seconds'] : '')
        );
