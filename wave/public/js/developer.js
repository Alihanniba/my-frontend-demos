...
developer.init = function() {
    ...
    $(document).on("keydown", function(event) {
        ...
        if (keycode === 13 && document.activeElement.id === "search-app") {
            window.location.href = BASEURL + 'admin/appList?sort=' + $('.app_list_total_module .app_list_by_classify .button_backgro').attr('type') +  '&kw=' + urlencode($('#search-app').val());
        }
    });
    $(".st_logo,.st_identity").on('click',function(event) {
        /* Act on the event */
        window.location.href = BASEURL + "admin/supporter";
    });
};

developer.initHeader = function() {
    var isOverAppSection = false;
    var isOverIndividualSection = false;
    var isOverUserPhone = false;
    $('ul.nav_container .app_section').hover(function() {
        /* Stuff to do when the mouse enters the element */
        isOverAppSection = true;
        $('#nav_app_menu').show();
    }, function() {
        /* Stuff to do when the mouse leaves the element */
        isOverAppSection = false;
        setTimeout(function() {
            if (!isOverAppSection) {
                $('#nav_app_menu').hide();
            }
    ...
    else if (now_url.indexOf("appList") > 0 || now_url.indexOf("appInfo") > 0) {
    ...
    $('.nav_addapp').hover(function() {
        $('.nav_addapp').attr('src', "./../images/addapphover.png");
        $('#nav_add_tip').show();
    }, function() {
        $('.nav_addapp').attr('src', "./../images/addapp.png");
        $('#nav_add_tip').hide();
    });
    ...
    // $(".message_center").mouseout(function(event) {
    ...
    // $('ul.nav_container li > a').click(function(event) {
    //     $('ul.nav_container li > a').removeClass('title_active');
     ...
     * Application Center
     */
    $('.supporter_nav_menu .app-add').click(function(event) {
        /* Act on the event */
        $('.nav_addapp').attr('handleType', 'addApp').click();
    });

    $('.supporter_nav_menu .app-modify').click(function(event) {
        /* Act on the event */
        $('.nav_addapp').attr('handleType', 'editApp').click().attr('handleType', 'addApp');
    });
     ...
     */
    developer.initAddApp();
};

developer.initDeveloper = function() {
    ...
    $('.basedata_yesterday').click(function(event) {
        var currentApp_id = $('.currentApp_name').attr('data-appid');
        //dateStatistic(currentApp_id);
    });
    $('.basedata_week').click(function(event) {
        ...
        var currentApp_id = $('.currentApp_name').attr('data-appid');
        //dateStatistic(currentApp_id, weekago, currentDate);
    });
    $('.basedata_month').click(function(event) {
        ...
        var currentApp_id = $('.currentApp_name').attr('data-appid');
        //dateStatistic(currentApp_id, monthago, currentDate);
    });

    $('.profile_yesterday').click(function(event) {
        var currentApp_id = $('.currentApp_name').attr('data-appid');
        DateCharts(1, currentApp_id);
    });
    $('.profile_week').click(function(event) {
        ...
        var weekago = new Date(date.getTime() - 7 * 24 * 3600 * 1000).format('yyyy-MM-dd');
        var currentApp_id = $('.currentApp_name').attr('data-appid');
        DateCharts(1, currentApp_id, weekago, currentDate);
    });
    $('.profile_month').click(function(event) {
        ...
        var monthago = new Date(date.getTime() - 30 * 24 * 3600 * 1000).format('yyyy-MM-dd');
        var currentApp_id = $('.currentApp_name').attr('data-appid');
        DateCharts(1, currentApp_id, monthago, currentDate);
    });
    $('.currentApp_name').click(function(event) {
        $('#app_listMenu').show();
    });
    $('.select_app').hover(function() {
    ...
    $('#app_listMenu').on('click', '.supporter_list_menu >a', function(event) {
        $('.currentApp_name').html($(this).html());
        $('#app_listMenu').hide();
        var this_option = $(this).attr('data-appid');
        $('.currentApp_name').attr('data-appid', this_option);
        ...
        //dateStatistic(this_option);
        DateCharts(1, this_option);
    });
    $('.date_rage').on('click', '.submitTime', function(event) {
        ...
        var currentApp_id = $('.currentApp_name').attr('data-appid');
        DateCharts(1, currentApp_id, startdate, enddate);
    });
    function list_allappName() {
        $('.supporter_list_menu').find('a').remove();
        $.ajax({
                url: BASEURL + 'admin/api/appNameList',
                type: 'GET',
                dataType: 'json'
            })
            .done(function(data) {
                if (data.success) {
                    for (var i = 0; i < data.result.length; i++) {
                        $('.supporter_list_menu').append('<a data-appID="' + data.result[i].id + '">' + data.result[i].app_name + '</a>');
                    }
                }
                $('.currentApp_name').html($('.supporter_list_menu a:first').html());
                var initappID  = $('.supporter_list_menu a:first').attr('data-appid');
                $('.currentApp_name').attr('data-appid', initappID);
                //dateStatistic(initappID);
                DateCharts(1, initappID);
    ...
    function dateStatistic(id, startdate, enddate) {
        ...
        $.ajax({
                url: BASEURL + 'admin/api/getAppStatisticInfo',
                type: 'POST',
                dataType: 'json',
                data: {
                    id: id,
            ...
            .done(function(data) {
                console.log(data);
                var rate = data.result.convertRate * 100;
                $('#acceptnum_act').html(data.result.receiveNum + '人');
                $('#clicknum_act').html(data.result.viewNum + '人');
                $('#ratenum_act').html( '0人');
                //$('#ratenum_act').css('width', '1%');
                var acceptbar = $('#acceptbar_act');
                if (data.result.receiveNum != 0) {
                    acceptbar.css('width', '70%');
                }else{
                     acceptbar.css('width', '1%');
                }
    ...
    function financeReport(){
        ...
        .done(function(data) {
          if (data.success) {
             ...
             $('.finance_total .finance_amount').html('￥'+data.totalSalary);
          }
        })
        .fail(function(data) {
            console.log("error");
    ...
    function getClickCount() {
        $.ajax({
            url: BASEURL + 'admin/api/TotalMessagesExposureAndClickNumByDeveloer',
            type: 'POST',
            dataType: 'json',
        })
        .done(function(data) {
...
developer.initIndividual = function() {
    var _loading = 0;
    var bank_type_id;
    ...
    //页面初始化即让银行卡输入框获取焦点,当失去焦点时自动获取银行卡类型和开户行
    //$("#input_finance_info #bank_card").focus();
    ...
    // $("#bank_card").focus(function(event) {
    //     $(".bank_card_warn .warning").hide();
    // });
...
/**
 * [initAddApp: Initiate add app page]
 * @return {[type]} [description]
 */
developer.initAddApp = function() {
    var handleType = {
        editApp: {
            title: '编辑应用',
            buttonName: '编辑',
            ...
            postFunc: function initEditBtn() {
                ...
                //modified by wangzhe 20160428
                checkPush();
                $("#addAppForm input[name=hasPush]").on("change", function(){
                    checkPush();
                });
                /** then bind */
                $('.dialog .send-btn').click(function(event) {
                    ...
                    /* Act on the event */
                    var formData = new FormData($('.dialog #addAppForm')[0]);
                    $.ajax({
                            url: BASEURL + 'admin/api/appInfoEdit',
                            type: 'POST',
                            dataType: 'json',
                            data: formData,
                            cache: false,
        ...
        addApp: {
            ...
            formID: 'addAppForm',
            buttonName: '添加',
            description: '<p>如果您是app开发商，请您正确填写app名称、平台信息及app的详细介绍，</p><p>我们将在24小时内给您审核。如果您是广告商，请您重新登陆。</p>',
            postFunc: function initAddBtn() {
                ...
                $('.dialog .send-btn').click(function(event) {
                    ...
                    /* Act on the event */
                    if ($('#addAppForm #app_name').val() == "") {
                        showError('请填写App应用名称');
                        return;
                    }

                    if ($('#addAppForm #app_description').val() == "") {
                        showError('请填写App介绍');
                        return;
                    } else {
                        if ($('#addAppForm #app_description').val().length > 200) {
                            showError('不能有多于200字的应用介绍');
                            return;
                        }
                    }

                    var selectedVal = $(".dialog .app-tab .active").attr('value');
                    switch (selectedVal) {
                        case '2':
                            {
                                if ($('#addAppForm #bundle-id').val() == "") {
                                    showError('请填写BundleID');
                                    return;
                                }

                                //modified by wangzhe 20160428
                                if($("#addAppForm input[name=hasPush]:checked").val() == "1"){
                                    if ($('#addAppForm #develop-certification').val() == "") {
                                        showError('请选择开发证书');
                                        return;
                                    }
                                    if ($('#addAppForm #develop-secret').val() == "") {
                                        showError('请填写开发证书密码');
                                        return;
                                    }
                                    if ($('#addAppForm #production-certification').val() == "") {
                                        showError('请选择生产证书');
                                        return;
                                    }
                                    if ($('#addAppForm #production-secret').val() == "") {
                                        showError('请填写生产证书密码');
                                        return;
                                    }
                                }
                        ...
                        case '3':
                            {
                                if ($('#addAppForm #package-name').val() == "") {
                                    showError('请填写应用包名');
                                    return;
                                };
                                break;
    ...
    //modified by wangzhe 20160428
     function checkPush() {
        if($("#addAppForm input[name=hasPush]:checked").val() == "0"){
            $(".push-content").hide();
        }else {
            $(".push-content").show();
        }
     ...
     */
    function removeAll() {
        $('.dialog .content input:not([type=radio]), .dialog .content textarea').val('');
        $('.dialog .content #ios-area .label-container label').removeClass('selected');
    }
            ...
            var dlg = new DialogFx(somedialog, {
                onCloseDialog: function() {
                    if ($('.dialog .back-btn').attr('refresh') == 'true') {
                        window.location.href = BASEURL + 'admin/appList';
                    }
                }
            });
            dlgtrigger.addEventListener('click', function() {
                var pageType = $(this).attr('handleType') || 'addApp';
                var appID = $(this).attr('appID') || '0';
                initInterface(pageType, appID, function() {
                    dlg.toggle.bind(dlg)();
                });
            });
        }
     ...
     * @return {[type]} [description]
     */
    function initInterface(pageType, appID, callback) {
        ...
        if (pageType == 'editApp') {
            ...
            /** get the basic app info which will be the first one when appID is 0 */
            $.ajax({
                    url: BASEURL + 'admin/api/getAppInfo',
                    type: 'POST',
                    dataType: 'json',
                    data: {
                        appID: appID
                ...
                .done(function(data) {
                    if (data.success) {
                        ...
                        $('.dialog #app_id').val(data.info.id);

                        $('.dialog .content #app_name').val(data.info.app_name);
                        $('.dialog .content #app_description').val(data.info.description);

                        //modified by wangzhe 20160428
                        if(data.info.has_push == "1"){
                            $('.dialog .content input[name=hasPush]').removeAttr('checked');
                            $('.dialog .content #hasPush1').prop('checked',true);
                            checkPush();
                        }else {
                            $('.dialog .content input[name=hasPush]').removeAttr('checked');
                            $('.dialog .content #hasPush0').prop('checked',true);
                            checkPush();
                        }

                        switch (data.info.terminal_system_id) {
                            case 2:
                                {
                                    /** IOS */
                                    $('.dialog .content #bundle-id').val(data.info.app_name);
                                    break;
                                }
                            case 3:
                                {
                                    /** Android */
                                    $('.dialog .content #package-name').val(data.info.package_name);
                                    break;
                                }
                            default:
                                break;
                        ...
                        $('.dialog .app-tab > ul > li').each(function(index, el) {
                            if ($(this).attr('value') == data.info.terminal_system_id) {
                                $(this).click();
                            }
                        });
            ...
            $('.dialog .app-tab > ul > li:first-of-type()').click();
    ...
    /**
     * [initAppChoose: init the tab area for choosing app]
     * @return {[type]} [description]
     */
    function initAppChoose() {
        /** choosing app */
        $('.app-tab ul > li').click(function(event) {
            /* Act on the event */
            /** add class of active */
            $('.app-tab ul > li').each(function() {
            ...
            $('#app_system_id').val($(this).attr('value'));
        });
    }
...
developer.initFinanceCenter = function(curPage, totalLen, perPage) {
    curPage = curPage || '1';
    totalLen = totalLen || '1';
    perPage = perPage || '1';
    ...
    // 点击遮罩关闭窗口
    $(".finance_withdraw_module .withdraw_shadow").on('click',  function(event) {
        $(".finance_withdraw_module .withdraw_box").fadeOut();
        $(".finance_withdraw_module .no_bank_card").fadeOut();
        $(".finance_withdraw_module .kefu_liaotian").fadeOut();
        $(".finance_withdraw_module .withdraw_shadow").fadeOut('fast');
    });
    $(".finance_total_module .finance_tatol_data_withdraw button").on('click',  function(event) {
        $.ajax({
            url: BASEURL + 'admin/api/getUserBankCard',
            type: 'POST',
            dataType: 'json',
        })
        .done(function(data) {
            if(data.errCode == 200){
                ...
                bank_card = bank_card.slice(-4);
                $("#money_to_bank span").text(data.bank_name + "("+ bank_card +")");
                $(".withdraw_shadow").fadeIn('fast');
                $(".kefu_liaotian").fadeIn('fast');
                $("#input_withdraw_info input").val("");
            }else if(data.errMsg == "找不到相应的银行卡号"){
                $(".withdraw_shadow").fadeIn('fast');
                $(".no_bank_card").fadeIn('fast');
            }
        })
        .fail(function(data) {
            console.log(data.errMsg);
    ...
    $(".withdraw_box button").on('click',  function(event) {
        ...
        var can_with_draw = $(".finance_total_data .data_balance_num span").text();
        if(!$("#money_to_bank span").text()){
        ...
        if(parseInt(money_cash) > parseInt(can_with_draw)){
            withdrawError("金额过大");
            return;
        }
        ...
        .done(function(data) {
            if(data.success == true){
                ...
                $("#withdraw_bank_card").text($("#money_to_bank span").text());
                $(".withdraw_box").fadeOut();
    ...
    $(".no_bank_card .go_set_bank").on('click', function(event) {
        window.location.href = BASEURL + 'admin/individual?go=financeSet';
    });
    ...
    function BalanceIncome(){
        ...
        .done(function(data) {
            $('.finance_tatol_data_income .data_income_num').html('￥'+data.totalSalary);
            $('.finance_tatol_data_balance .data_balance_num').html('￥'+data.balance);
        })
        .fail(function() {
            console.log("error");
        })
...
developer.initAppList = function(curPage, totalLen, perPage) {
    curPage = curPage || '1';
    totalLen = totalLen || '1';
    perPage = perPage || '1';

    //切换应用列表show条件
    $(".app_list_total_module .app_list_by_classify .list_classify").on('click', function(event) {
        event.preventDefault();
        /* Act on the event */
        window.location.href = BASEURL + 'admin/appList?sort=' + $(this).attr('type');
    });
    if (parseInt(totalLen) > parseInt(perPage)) {
        $("div.app_list_paging").jPages({
            containerID: "itemContainer",
            // pause       : 1000,
            clickStop: true,
            totalLen: parseInt(totalLen),
    ...
    $('.app_list_total_module .app_operate .app_info_edit').click(function(event) {
        /* Act on the event */
        $('.nav_addapp').attr('handleType', 'editApp').attr('appID', $(this).attr('appID')).click().attr('handleType', 'addApp').attr('appID', '');
    });

    $('.app_list_total_module .app_operate .app_info_delete').on('click', function(event) {
        /* Act on the event */
        var id = $(this).attr('appID');
        var name = $(this).parent().siblings('.app_name').html();
        if (deleteData(name)) {
            $.ajax({
                    url: BASEURL + 'admin/api/appInfoDelete',
                    type: 'POST',
                    dataType: 'json',
                    data: {
                        id: id
    ...
    $('.app_list_total_module .app_list_by_classify .app_list_search input + span').click(function(event) {
        /* Act on the event */
        window.location.href = BASEURL + 'admin/appList?sort=' + $('.app_list_total_module .app_list_by_classify .button_backgro').attr('type') +  '&kw=' + urlencode($(this).prev().val());
    });
    $('.app_list_total_module #itemContainer .app_data_info').on('click', '.app_platform, .app_key, .app_name, .app_package_name, .app_data', function(event) {
        event.preventDefault();
        window.location.href = BASEURL + 'admin/appInfo?appID=' + urlencode($(this).parent().attr('appID'));
    });
...
developer.initAppInfo = function(appID) {
    appID = appID || '1';
    appStatics(appID);
    ...
    $('#copy_app_key').zclip({
        ...
        afterCopy: function(){
            $("#copy_app_key #msg").show().fadeOut(2000);
        }
    });
    $('#copy_app_master').zclip({
        path: '../js/ZeroClipboard.swf',
        copy: function(){
            return $('#app_master').text();
        },
        afterCopy: function(){
    ...
    $('.date_rage').on('click', '.submitTime', function(event) {
        var startdate = $(this).parent().find("#timeStart").val();
        var enddate = $(this).parent().find("#timeEnd").val();
        DateCharts(1, appID, startdate, enddate);
    });
    ...
    function appStatics(appID) {
        $.ajax({
            ...
            .done(function(data) {
                ...
                if (data.success) {
                    $('#app_basic_data').html('');
                    $('#app_basic_data').append('<p>基本指标&nbsp;&nbsp;(昨日)</p>\
                      <div>\
                          ...
                          <ul class="basic_data_num">\
                              <li>' + data.result.usersCount + '人</li>\
                              <li>' + data.result.yesterdayUsersCount + '人</li>\
                          </ul>\
                      </div>\
                      ');
                      //<li>推送量</li>\
                      //<li>' + data.result.yesterdayPushTime + '次</li>\
                    // <div>\
                    //       <ul class="basic_param">\
                    //           <li>接收数</li>\
                    //           <li>点击数</li>\
                    ...
                    //       </ul>\
                    //       <ul class="basic_data_num">\
                    //           <li>' + data.result.receiveNum + '人</li>\
                    //           <li>' + data.result.viewNum + '人</li>\
                    //           <li>0 人</li>\
                    //       </ul>\
                    //   </div>
                }
...
/**
 * [initUploader: Initiate qiniu uploader]
 * @param  {[type]} appID [app id]
 * @return {[type]}       [description]
 */
developer.initUploader = function(appID) {
    appID = appID || '1';
    ...
    var showUploadInfos = function(up, file) {
        if (!isHover) {
            $('.app_info_module .upload-file .upload-status').text(file.percent + '%');
        }
    };

    var uploader = Qiniu.uploader({
        runtimes: 'html5,html4',
        browse_button: 'pick-apk',
        container: 'local-apk-upload',
        ...
        init: {
            'FilesAdded': function(up, files) {
                ...
                plupload.each(files, function(file) {
                    ...
                    // 文件添加进队列后,处理相关的事情
                    var _file = file;
                    var $btn = $('.app_info_module .upload-file .upload-btn');
                    var $status = $('.app_info_module .upload-file .upload-status');

                    /**
                     * unillegal files
                     */
                    if (_file.name.toLowerCase().indexOf('apk') < 0) {
            ...
            'FileUploaded': function(up, file, info) {
                ...
                $.ajax({
                    url: BASEURL + 'admin/api/uploadFile',
                    type: 'POST',
                    dataType: 'json',
                    data: {
                        'appID': appID,
                        'url': domain + infoObj.key
                    }
                })
                .done(function() {
                    $status.unbind('click')
                ...
                .fail(function() {
                    console.log('error to update uri');
                });
            },
 ...
 * [initAppLog: sdk日志]
 */
developer.initAppLog = function(curPage, totalLen, perPage){
    // 打开窗口
    $('.his_data_list .more').on('click', function(event) {
        // console.log($(this).parent().next().val());
        $('#see_des_module .des_content').text($(this).parent().next().val());
        $(".withdraw_shadow").fadeIn('fast');
        $(".see_des_box").fadeIn('fast');
    });
    // 点击关闭按钮关闭窗口
    $(".see_des_box .close_btn").on('click',  function(event) {
        $(this).parent().fadeOut();
        $(".withdraw_shadow").fadeOut('fast');
    });
    // 点击遮罩关闭窗口
    $("#see_des_module .withdraw_shadow").on('click',  function(event) {
        $(".see_des_box").fadeOut();
        $(".withdraw_shadow").fadeOut('fast');
    });
    ...
    if (parseInt(totalLen) > parseInt(perPage)) {
        console.log($("div.his_list_paging"));
        $("div.his_list_paging").jPages({
            containerID: "itemContainer",
            // pause       : 1000,
            clickStop: true,
            totalLen: parseInt(totalLen),
