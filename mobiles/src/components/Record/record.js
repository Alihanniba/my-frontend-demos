...
class Record extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            iconState:false,
            iconExit:false,
            na:[],//[{titleId:8937,episodeId:8989,name: '1', img:'https://i.ytimg.com/vi/8GNkY6pHlds/mqdefault.jpg',duration:124, current:676.9, time:Date.now()}]
            itemDelete:[],
            colorMoc:[]
        }
        this.item=[],
    ...
    componentWillMount() {
        Storage.s_set(Storage.KEYS.RECORD_PAGE_KEY, true);
        if(!!!this.startPos){
            !!!Storage.get(Storage.KEYS['HISTORY']) ? this.mocData = [] : this.mocData = Storage.get(Storage.KEYS['HISTORY'])
            this.startPos += 1
    ...
    ListClick(index){
        if (this.state.iconState){
            ...
            hashHistory.push('/watch/'+this.state.na[index]['titleId']+"?eid=" + this.state.na[index]['episodeId'] + "&s=" + this.state.na[index]['current'])
    ...
    clearLabelClick(){
        if (this.state.itemDelete.length!==0){
            ...
            Storage.deleteItems(Storage.KEYS['HISTORY'],val)
            Storage.delete(Storage.KEYS['HISTORY'],function(item){
        ...
        if (this.item.length==0){
            ...
            return(
                this.state.na.map((item,i)=>{
                    ...
                    recordTool.formatSeconds(this.state.na[i]['duration']) !== recordTool.formatSeconds(this.state.na[i]['current']) ? durateName = '播放至 ' + recordTool.formatSeconds(this.state.na[i]['current']) :  durateName = '已看完'
                    recordTool.formatSeconds(this.state.na[i]['duration']).length > 5 ? sty = {} : sty = {width:'0.95rem',right:'-0.8rem'}
                    !!recordTool.formatSeconds(this.state.na[i]['duration']).length ? sty = sty : sty = {display:"none"}
                    return(
                    <div className='li-list' key={i} onClick={()=>this.ListClick(i)} >
                        ...
                        <div className='center-container-time'>
                            <div className='center-container-time-space' style={sty}>
                                {recordTool.formatSeconds(this.state.na[i]['duration'])}
                            </div>
                        </div>
                        <div className='right-container'>
                            <div className='right-container-text'>{this.state.na[i]['name'].slice(0,30)}</div>
                            <div className='right-container-time'>{durateName}</div>
                        </div>
                    </div>
                    )
    ...
    render() {
        let titleBar  = titleBarInitNoOnseUi(this.leftLabelClick,this.rightLabelClick,this.state.iconState,!!this.state.na.length,"观看记录")
        ...
        if (!!!this.state.na.length){
            return (
                <div className='container'>
                    {titleBar}
                    <div style={{lineHeight:"16rem",textAlign:"center"}}>客官您还没有观看过任何视频</div>
                </div>
                )
        }
        return (
