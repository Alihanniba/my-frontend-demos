...
class Youku {
    ...
    public static function _getYouku($vid){
        ...
        if ($retval) {
            ...
            //返回 图片 标题 链接  时长  视频地址
            $img = json_decode(Base::_cget('https://openapi.youku.com/v2/videos/show_basic.json?client_id=3996367281b3c288&video_id=' . $rs['data'][0]['videoid']), true);

            $data['img'] = $img['thumbnail_v2'];
            $data['title'] = $rs['data'][0]['title'];
            $data['seconds'] = $rs['data'][0]['seconds'];
//            var_dump($rs['data'][0]['segs']);
            if(isset($rs['data']['0']['segs']['mp4']))
                $data['segaments']['high'] = $rs['data'][0]['segs']['mp4'];
            else
                $data['segaments']['high'] = $rs['data'][0]['segs']['3gphd'];
            if(isset($rs['data']['0']['segs']['flv']))
                $data['segaments']['normal'] = $rs['data'][0]['segs']['flv'];
            if(isset($rs['data']['0']['segs']['hd2']))
                $data['segaments']['super'] = $rs['data'][0]['segs']['hd2'];
//            else
//                $data['segaments'] = $rs['data'][0]['segs']['flv'];
            return $data;
        } else {
            return false;
        }
