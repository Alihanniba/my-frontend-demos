...
class Iqiyi
{
    public static $deadpara = 10000;
    public static $enc_key = '2c76de15dcb44bd28ff0927d50d31620';
    private static $reservedArray = array();
    ...
    {
        $api_url = 'http://cache.video.qiyi.com/vms?key=fvip&src=1702633101b340d8917a69cf8a4b8c7';
        ...
        if (is_array($vs) && count($vs) > 0) {
            ...
            $getDurationUrl = 'http://videocreate.soundtooth.cn/ffmpeg/getVideoInfo';
            $urls = '';
            $urls = isset($data['fluent'])? $urls . ',' . implode(',', $data['fluent']) : $urls;
            $urls = isset($data['normal'])? $urls . ',' . implode(',', $data['normal']) : $urls;
            $urls = isset($data['high'])? $urls . ',' . implode(',', $data['high']) : $urls;
            $urls = isset($data['super'])? $urls . ',' . implode(',', $data['super']) : $urls;
            $urls = substr($urls, 1, strlen($urls) - 1);
            $postData = array(
            	'urls' => $urls
            );
            $result = self::curl($getDurationUrl, $postData)['data'];
            self::createReservedArray('fluent', $data);
            self::createReservedArray('normal', $data);
            self::createReservedArray('high', $data);
            self::createReservedArray('super', $data);
            for($i = 0; $i < sizeof($result) ; $i++) {
            	$segaments[self::$reservedArray[$i]][] = array (
                    'seconds' => $result[$i]['intDuration'],
                );
            }
            $data['segaments'] = $segaments;
//            var_dump(self::gettime($data['fluent'][0]));exit;
    ...
    /**
     * [createReservedArray description]
     * @return [type] [description]
     */
    private static function createReservedArray($name, $data) {
    	if(isset($data[$name])) {
    		for($i = 0; $i < sizeof($data[$name]); $i++) {
	    		self::$reservedArray[] = $name;
	    	}
    	}
    ...
    private static function curl($postUrl, $postData) {
    	// var_dump($postUrl);
